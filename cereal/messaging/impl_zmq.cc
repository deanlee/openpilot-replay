...
int ZMQSubSocket::connect(Context *context, std::string endpoint, std::string address, bool conflate, bool check_endpoint, size_t segment_size){
  ZMQContext *zmq_context = (ZMQContext*)(context);
  assert(zmq_context);
  sock = zmq_socket(zmq_context->getRawContext(), ZMQ_SUB);
  if (sock == NULL){
    return -1;
  }
...
int ZMQPubSocket::connect(Context *context, std::string endpoint, bool check_endpoint, size_t segment_size){
  ZMQContext *zmq_context = (ZMQContext*)(context);
  assert(zmq_context);
  sock = zmq_socket(zmq_context->getRawContext(), ZMQ_PUB);
  if (sock == NULL){
    return -1;
  }
...
void ZMQPoller::registerSocket(SubSocket * socket){
  ...
  ZMQSubSocket *zmq_socket = dynamic_cast<ZMQSubSocket*>(socket);
  assert(zmq_socket);
  polls[num_polls].socket = zmq_socket->getRawSocket();
